...
------- FUNCION BUSCAR HISTORIAL -------
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID('[LOS_JUS].[buscarHistorial]'))
DROP FUNCTION LOS_JUS.buscarHistorial
GO

CREATE FUNCTION LOS_JUS.buscarHistorial
(
...
RETURN (
	...
	WHERE PUB.PUB_CODIGO NOT IN (SELECT OPE_CODIGO FROM LOS_JUS.OPERACION WHERE OPE_PUBLICACION = PUB.PUB_CODIGO)
		...
		AND PUB.PUB_FECHA_INICIO >= CONVERT(datetime,CASE WHEN @anio IS NOT NULL AND @mes IS NOT NULL THEN @anio + '-01-' + @mes + ' 00:00:00.000' ELSE PUB.PUB_FECHA_INICIO END) 

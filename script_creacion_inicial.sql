...
------- FUNCION BUSCAR PUBLICACIONES ACTIVAS-------

IF EXISTS 
(SELECT * FROM sys.objects 
WHERE object_id = OBJECT_ID('[LOS_JUS].[publicacionesActivas]') 
AND type in ('FN', 'IF', 'TF', 'FS', 'FT'))
DROP FUNCTION LOS_JUS.publicacionesActivas
GO

CREATE FUNCTION LOS_JUS.publicacionesActivas
(
@id integer = NULL
)
RETURNS TABLE
...
AS 
RETURN (
	SELECT OPE.*, FAC.*, PUB.PUB_DESCRIPCION
	FROM LOS_JUS.OPERACION OPE
		LEFT JOIN LOS_JUS.FACTURACION FAC ON OPE.OPE_CODIGO = FAC.FAC_OPERACION  
		JOIN LOS_JUS.PUBLICACION PUB ON OPE.OPE_PUBLICACION = PUB.PUB_CODIGO
	WHERE OPE.OPE_CLIENTE = ISNULL(@id, OPE.OPE_CLIENTE)
		AND FAC.FAC_NUMERO IS NULL
		AND OPE.OPE_ELIMINADO = 0
   )
GO
